...
parser.add_argument(
    "--model_path", type=str, default="DarkFlameUniverse/Stable-Diffusion-2-1-Base-8bit",
...
try:
    ...
    print("Enabling vae and attention slicing...")
    pipeline.enable_vae_slicing()
    pipeline.enable_attention_slicing()
    pipeline.to("cuda")
except Exception as e:
    logging.error(f"Model loading failed: {str(e)}, switching to standard 2-1 base.")
    # Fallback to non-quantized
    pipeline = DiffMorpherPipeline.from_pretrained(
        "stabilityai/stable-diffusion-2-1-base",
        torch_dtype=torch.float32
    )
    print("Fallback model: Enabling vae and attention slicing...")
    pipeline.enable_vae_slicing()
    pipeline.enable_attention_slicing()
    pipeline.to("cuda")
